...
$stmt->fetch();
$stmt->close();
$amt=$coun*150; //echo$amt;//$_SESSION['amt']=$amt; echo $_SESSION['amt'];
$rt=mysqli_fetch_array(mysqli_query($mysqli,"select sum(withdrawlAmount) from withdrawl where cid='$aid'"),MYSQLI_BOTH);
if(!empty($rt['sum(withdrawlAmount)'])){ $a=$amt-$rt['sum(withdrawlAmount)']; echo $a; }else{echo"Not Found";}
$t=mysqli_query($mysqli,"select * from withdrawl where cid='$aid'");
	$c=mysqli_num_rows($t);
	if($c>0)
	{
      $u=mysqli_query($mysqli,"update withdrawl set walletAmount='$a' where cid='$aid'");
	}
else{
$sq=mysqli_query($mysqli,"insert into withdrawl(cid,walletAmount) values('$aid','$a')");
}
